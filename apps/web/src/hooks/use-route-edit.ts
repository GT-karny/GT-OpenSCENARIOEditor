 ...
 */
export function useRouteEdit(odrDoc: OpenDriveDocument | null) {
  ...
  useEffect(() => {
    ...
    for (const wp of store.editingRoute.waypoints) {
      ...
      if (world) {
        ...
        // Fallback: zero position for unresolvable positions
        positions.push({ x: 0, y: 0, z: 0, h: 0 });
      }
    }
    store.setWaypointWorldPositions(positions);

    // Compute path segments (straight lines for now; will upgrade to WASM pathfinding)
    ...
    // If closed route, connect last → first
    if (store.editingRoute.closed && positions.length >= 2) {
  ...
  }, [store.editingRoute, odrDoc, store.active]);
