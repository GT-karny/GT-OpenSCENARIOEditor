...
| 対象規格 | OpenSCENARIO XML v1.2 / OpenDRIVE v1.6 |
...
│         │         └────────^─────────┘                   │
│  ┌──────┴──────┐  ┌────────┴─────────┐                   │
│  │@osce/       │  │@osce/node-editor │                   │
│  │ 3d-viewer   │  │ React Flow       │                   │
│  │ Three.js描画│  │ カスタムノード    │                   │
│  │ 道路/車両   │  │ タイムライン同期  │                   │
│  └─────────────┘  └──────────────────┘                   │
│                                                          │
...
```
ブラウザ (React)
  ├── WebSocket ──→ apps/server (Node.js)
  │                    ├── child_process.spawn → esmini.exe (headless)
  │                    ├── ファイルシステム (.xosc, .xodr)
  │                    └── MCP Server (stdio/SSE)
  │
  ├── Zustand Store (ブラウザ内状態)
  │     ├── @osce/scenario-engine (データモデル、コマンド)
  │     ├── @osce/openscenario (パース/シリアライズ)
  │     └── @osce/opendrive (道路パース)
  │
  ├── React Flow (ノードエディタパネル)
  ├── Three.js Canvas (3Dビューアパネル)
  └── タイムラインパネル (同期ビュー)
```
...
esminiサブプロセス管理、.datファイルパース、シミュレーション結果ストリーミング。
- 依存: `@osce/shared`（サーバーサイドのみ）
- 参照: `Thirdparty/esmini-demo_Windows/esmini-demo/EnvironmentSimulator/Libraries/esminiLib/esminiLib.hpp`
...
Node.jsバックエンド。ファイルI/O、esminiプロセス管理、WebSocket通信。
...
アクションレベル（上級者向け）
  「加速」「車線変更」「停車」...
    ...
    ▼
OpenSCENARIO要素
  Story → Act → ManeuverGroup → Maneuver → Event → Action + Trigger
```
...
[Storyboard]                          Time ──────────────────→
  ├─[Init]
  │   ├─[Ego: Teleport+Speed]        Ego:  ███ 20m/s ████████████>
  │   └─[A1: Teleport+Speed]
  ├─[Story: CutIn]                    A1:   ███ 25m/s ███[LaneChg]█>
  │   └─[Act]─[Trigger]                              ↑
  │       └─[ManeuverGroup]─[Actor:A1]        TriggerA(距離<20m)
  │           └─[Maneuver]
  │               └─[Event]─[Trigger]
  │                   └─[Action: LaneChange]
  └─[StopTrigger]
...
### 4. esmini連携（サブプロセス + .dat + WebSocket）

```
ブラウザ                    Node.jsサーバー              esmini.exe
  │                            │                          │
  │── "simulate" ─────────────→│                          │
  │                            │── spawn ────────────────→│
  │                            │   --headless --osc       │
...
| トラック | パッケージ | 主な作業 |
|---------|-----------|---------|
| H | `apps/server` | Fastifyサーバー、WebSocket、ファイルI/O |
| I | `@osce/esmini` | サブプロセス管理、.datパーサー、ストリーミング |
...
| `Thirdparty/esmini-demo_Windows/esmini-demo/EnvironmentSimulator/Libraries/esminiLib/esminiLib.hpp` | esmini C API定義 |
...
7. **esmini**: サンプルシナリオを実行 → 3Dビューアでアニメーション再生
...
```
@osce/shared          ← 依存なし (純粋型定義)
    ^
    |
    +-- @osce/openscenario     ← shared, fast-xml-parser
    +-- @osce/opendrive        ← shared, fast-xml-parser
    +-- @osce/scenario-engine  ← shared, zustand, immer, uuid
    +-- @osce/i18n             ← shared, i18next, react-i18next
    +-- @osce/templates        ← shared
    +-- @osce/esmini           ← shared (server-side only)
    |
    +-- @osce/node-editor      ← shared, scenario-engine, @xyflow/react, dagre
    +-- @osce/3d-viewer        ← shared, opendrive, scenario-engine, three, @react-three/fiber
    +-- @osce/mcp-server       ← shared, scenario-engine, @modelcontextprotocol/sdk
    |
    +-- apps/web               ← ALL frontend packages
    +-- apps/server            ← shared, openscenario, esmini, mcp-server
```
