...
ASAM OpenSCENARIO XML v1.3.1のシナリオを、Webブラウザ上でグラフィカルに作成・編集できるエディタ。OpenSCENARIOに詳しくないユーザーでも直感的にシナリオを構築でき、AIエージェントとの協調作業も可能なツールを目指す。
...
| 対象規格 | OpenSCENARIO XML v1.3.1 / OpenDRIVE v1.6 / ASAM OSI |
...
graph LR
  ...
  Browser -- "REST /api/projects\n+ WS" --> Server
  WS --> FS
  WS -. "planned / not wired" .-> MCP
  WS -. "@osce/esmini\n(planned / not wired)" .-> REST
  WS -. "@osce/esmini\n(planned / not wired)" .-> GRPC
  REST --> ESMINI
  GRPC --> ESMINI
```
...
全パッケージが依存する型定義・インターフェース。**外部依存なし。** 中身は型定義に加え、XSD由来のconst配列（`COORDINATE_SYSTEMS` / `DYNAMICS_SHAPES` / `RULES` 等、UIのオプションリスト生成に使用）と、少量のユーティリティ（`generateId`、エンティティカラー）を含む。
...
- テストデータ: `Thirdparty/openscenario-v1.3.1/`, `Thirdparty/openscenario-v1.2.0/Examples/*.xosc`, `Thirdparty/esmini-demo_Windows/esmini-demo/resources/xosc/`
...
Three.jsによるOpenDRIVE道路描画 + シナリオエンティティ表示 + 交通信号3Dレンダリング + ルート編集 + ミニマップ + シミュレーション再生。
InstancedMeshによる描画最適化、共有ジオメトリ/マテリアルによるメモリ効率化。
- 依存: `three`, `@react-three/fiber`, `@react-three/drei`, `@osce/shared`, `@osce/opendrive`, `@osce/scenario-engine`, `@osce/opendrive-engine`（交通信号の分類・カタログ・シェイプパスを共有）
...
### `apps/desktop` (@osce/desktop)
Electronデスクトップアプリケーション。`@osce/web` をパッケージ依存として取り込むのではなく、electron-builder の `extraResources` でビルド済みの `web/dist` をバンドルし、Electronメインプロセス内で `@osce/server` の `buildApp()` を起動して同フロントエンドを静的配信する（本番時は `@fastify/static` でSPAフォールバック）。
- 依存（実行時）: `@osce/server`（メインプロセスは esbuild でバンドルされワークスペースパッケージはインライン化されるが、宣言は実態に合わせ dependencies）, `fastify`, `@fastify/static`, `@fastify/websocket`, `@fastify/multipart`, `@fastify/cors`, `@grpc/grpc-js`, `electron-store` 等
- devDependency: `electron`, `electron-builder`

### `apps/server` (@osce/server)
Node.js（Fastify）バックエンド。**現状の依存は `@osce/shared` のみ。** 実装済みスコープはプロジェクトファイルI/O（`/api/projects`）+ 設定（`/api/settings`）+ WebSocket（`/ws`、現状は ping/pong スタブのみ）。
- 依存: `@osce/shared`, `fastify`, `@fastify/websocket`, `@fastify/multipart`, `@fastify/cors`, `adm-zip`
- **計画/未配線**: `@osce/openscenario` / `@osce/esmini`（GT_Sim連携）/ `@osce/mcp-server` は現時点で未接続。
...
| ファイル | 用途 |
|---------|------|
| `Thirdparty/openscenario-v1.3.1/ASAM_OpenSCENARIO_v1.3.1_Schema/OpenSCENARIO.xsd` | 全OpenSCENARIO型の権威的ソース（v1.3.1、プライマリ） |
| `Thirdparty/openscenario-v1.2.0/Schema/OpenSCENARIO.xsd` | OpenSCENARIO v1.2スキーマ（後方互換用） |
