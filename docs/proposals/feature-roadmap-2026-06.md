...
- **フェーズ1・2 完了**（2026-07-02）: A1-A6 / B1 / B2 / B4 / B5 — 詳細は tech-debt-audit の進捗節
- **フェーズ3 完了**（2026-07-02、順序変更あり）: 当初案の C-track 中心から組み替え、**D1 LHT を前倒しで完遂**（junction/taper/routing の rule 尊重、走行方向矢印、デフォルト rule UX。※「header の rule 属性」は仕様誤認だったため per-road rule + エディタ既定値として実装 — lht-support.md 参照）、**C3 パラメータ分布**（全14型 + 分布アタッチ UI + バリアントプレビュー）、**C4 バッチ実行マトリクス**（WASM ワーカープール並列実行 + 衝突/minTTC 判定 + ドリルダウン再生）も完遂。負債 Phase 4（God 分解 + undo パッチ化）同時完了
- **フェーズ4 へ後ろ倒し**: C1/C2 MCP 実接続 + safe-apply、C5 NL 生成ループ（C1 依存）、D2 Traffic アクション、D3 シナリオライブラリ、B3 XSD バリデーション完成
