...
Generated: 2026-06-12T23:29:13.224Z
...
| `core.param_variable_modify_mapping` | core | Parameter/Variable modifyマッピング整合 | ParameterAction/VariableAction の modify ルールが入出力で一致し、変換時の意味ずれを防ぐ機能。 | OpenSCENARIO編集コアの観点で「Parameter/Variable modifyマッピング整合」を実運用レベルまで明確化し、状態を継続監視できるようにする。 | L3 | L1 | L1 | gap | `packages/openscenario/src/serializer/build-actions.ts` | `packages/openscenario/src/__tests__/round-trip/round-trip.test.ts`<br>`packages/openscenario/src/__tests__/round-trip/action-position-round-trip.test.ts` | - | - | - 2026-06-13 証跡追記: ParameterAction/VariableAction modify rule (AddValue/MultiplyByValue) の往復バグ修正済み (build-actions.ts:386付近)。L1→L2昇格条件は充足しているがowner確認が必要 | core-team | 2026-03-12 |
...
| `core.import_error_recovery` | core | インポート失敗時の回復導線 | インポート失敗時に原因を提示し、再試行や代替操作へ戻せる回復導線を提供する機能。 | OpenSCENARIO編集コアの観点で「インポート失敗時の回復導線」を実運用レベルまで明確化し、状態を継続監視できるようにする。 | L3 | L2 | L2 | partial | `apps/web/src/hooks/use-file-operations.ts`<br>`apps/web/src/components/editor/AutosaveRecoveryDialog.tsx`<br>`apps/web/src/hooks/use-unsaved-changes-guard.ts` | `apps/web/src/__tests__/hooks/use-file-operations.test.tsx` | - | - | - 2026-06-13 証跡追記: ファイルopen/parseエラーtoast実装済み; autosave crash recoveryダイアログ追加 (AutosaveRecoveryDialog); unsaved-changes guard実装済み。E2Eカバレッジ追加で L3昇格可能 | core-team | 2026-03-28 |
| `core.command_undo_redo_integrity` | core | Undo/Redo整合性 | 複数操作のUndo/Redoで状態遷移が破綻しないことを保証し、編集作業の可逆性を担保する機能。 | OpenSCENARIO編集コアの観点で「Undo/Redo整合性」を実運用レベルまで明確化し、状態を継続監視できるようにする。 | L3 | L2 | L2 | partial | `packages/scenario-engine/src/store/scenario-store.ts`<br>`packages/opendrive-engine/src/store/opendrive-store.ts`<br>`packages/opendrive-engine/src/commands/command-history.ts` | `packages/scenario-engine/src/__tests__/command-history.test.ts` | - | - | - 2026-06-13 証跡追記: OpenDRIVE側redoがコマンドクラス配線により修正済み (opendrive-store.ts)。複合編集ケースの回帰網羅を増やす必要がある | core-team | 2026-03-14 |
...
| `sim.error_surface_to_ui` | sim | シミュレーションエラー可視化 | 実行時エラーをUIへ正規化して通知し、原因把握と回復判断を支援する機能。 | シミュレーション連携の観点で「シミュレーションエラー可視化」を実運用レベルまで明確化し、状態を継続監視できるようにする。 | L3 | L2 | L2 | partial | `apps/web/src/hooks/use-websocket.ts`<br>`apps/web/src/hooks/use-file-operations.ts` | `apps/web/src/__tests__/hooks/use-validation.test.tsx` | - | - | - 2026-06-13 証跡追記: ファイル open/parseエラーおよびカタログ自動ロード警告のtoast通知が実装済み (use-file-operations.ts)。E2Eスイート修復中のためL3昇格はオーナー確認待ち | sim-team | 2026-03-23 |
...
| `ux.error_recovery_flow` | ux | エラー時の回復フロー | 失敗時に復旧手順を提示し、ユーザーが編集状態を維持したまま回復できる機能。 | エディタUXの観点で「エラー時の回復フロー」を実運用レベルまで明確化し、状態を継続監視できるようにする。 | L3 | L2 | L2 | partial | `apps/web/src/components/ErrorBoundary.tsx`<br>`apps/web/src/components/editor/AutosaveRecoveryDialog.tsx`<br>`apps/web/src/hooks/use-unsaved-changes-guard.ts`<br>`apps/web/src/hooks/use-autosave.ts` | `apps/web/src/__tests__/hooks/use-file-operations.test.tsx`<br>`apps/web/src/__tests__/lib/autosave-snapshot-mapping.test.ts`<br>`apps/web/src/__tests__/lib/autosave-debounce-scheduler.test.ts` | - | - | - 2026-06-13 証跡追記: autosave crash recovery dialog, unsaved-changes guard (browser beforeunload + Electron native dialog), file-open/parseエラーtoast が実装済み。E2E カバレッジ追加でL3昇格可能 | web-team | 2026-03-25 |
